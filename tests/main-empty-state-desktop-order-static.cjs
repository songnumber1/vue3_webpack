...
const mainEmptyPath = path.join(
  root,
  "src/components/workspace/MainEmptyState.vue"
);
...
const suggestionsIndex = source.indexOf(
  'class="suggestion-row suggestion-row--between main-empty-state__suggestions'
);

assert(
  composerIndex !== -1,
  "desktop composer slot must exist in MainEmptyState"
);
assert(
  suggestionsIndex !== -1,
  "suggestions block must exist in MainEmptyState"
);
assert(
...
assert(
  source.includes(".desktop-center-prompt") &&
    source.includes("order: 10 !important;"),
