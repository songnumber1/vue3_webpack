<template>
  <div class="studio-workspace__scroll tw-box-border tw-flex tw-min-h-0 tw-flex-1 tw-flex-col tw-overflow-hidden tw-bg-studio-bg tw-p-[32px_clamp(16px,4vw,56px)_18px] tw-text-studio-text">
    ...
    <div v-if="activeTab === 'all'" class="studio-category-chips tw-mx-auto tw-mb-3 tw-mt-[-4px] tw-flex tw-w-full tw-max-w-studio tw-shrink-0 tw-flex-wrap tw-items-center tw-gap-2 tw-overflow-visible" :aria-label="t('mcp.categoryLabel')">
      <button
    ...
    <label v-if="activeTab === 'mine'" class="mcp-created-filter tw-inline-flex tw-shrink-0 tw-cursor-pointer tw-select-none tw-items-center tw-gap-2 tw-self-start tw-rounded-[8px] tw-border tw-border-solid tw-border-studio-border tw-bg-studio-panel tw-px-3 tw-py-[9px] tw-text-[14px] tw-font-semibold tw-leading-[1.35] tw-text-studio-text">
      <input
        :checked="createdOnly"
    ...
    <nav class="studio-pagination tw-mx-auto tw-mt-[14px] tw-flex tw-w-full tw-shrink-0 tw-items-center tw-justify-center tw-gap-1.5 tw-border-t tw-border-studio-border tw-pt-[14px]" :aria-label="t('mcp.pagination.label')">
      ...
      <button class="studio-page-icon-button tw-inline-flex tw-h-[34px] tw-min-w-[34px] tw-items-center tw-justify-center tw-rounded-studio tw-border tw-border-solid tw-border-studio-border tw-bg-studio-surface tw-text-inherit disabled:tw-cursor-not-allowed disabled:tw-opacity-45" type="button" :disabled="currentPage === maxPage" :aria-label="t('mcp.pagination.next')" @click="$emit('go-page', currentPage + 1)">
        <span class="studio-icon studio-icon--page-next" aria-hidden="true"></span>
      </button>
      <button class="studio-page-icon-button tw-inline-flex tw-h-[34px] tw-min-w-[34px] tw-items-center tw-justify-center tw-rounded-studio tw-border tw-border-solid tw-border-studio-border tw-bg-studio-surface tw-text-inherit disabled:tw-cursor-not-allowed disabled:tw-opacity-45" type="button" :disabled="currentPage === maxPage" :aria-label="t('mcp.pagination.last')" @click="$emit('go-page', maxPage)">
