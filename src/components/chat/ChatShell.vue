/** * @file ChatShell.vue * @description Vue component used in the chat web
  ...
  >
    ...
    <main class="chat-workspace">
      ...
      <template v-else>
        ...
        <button
          ...
          type="button"
          aria-label="맨 아래로 이동"
          @click="scrollBottom({force: true, behavior: 'smooth', stable: true})"
        >
          ↓
        </button>
        <PromptInput
    ...
    >
      <button
        ...
        type="button"
        @click="selectAssistantFromSheet(assistant.id)"
      >
...
const activeHistoryId = computed(() =>
  props.mode === "chat" ? route.params.id : null
);
...
function updateScrollBottomButton() {
  showScrollBottom.value =
    props.mode === "chat" &&
    Boolean(listRef.value && !listRef.value.isAtBottom?.());
}
