    ...
    <transition name="sheet-slide">
      ...
      >
        ...
        <header class="bottom-sheet-header">
          <h2>{{ title }}</h2>
          <button type="button" class="bottom-sheet-close" aria-label="닫기" @click="emit('close')">×</button>
        </header>
...
function getSafeBottom() {
  ...
  probe.style.cssText = 'position:fixed;bottom:env(safe-area-inset-bottom);height:0;visibility:hidden;'
...
function clampHeight(height) {
  ...
  const maxHeight = Math.max(props.minHeight, Math.floor(viewportHeight * props.maxRatio) - getSafeBottom())
