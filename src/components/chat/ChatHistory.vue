...
import {
  appendUserAndChatResponses as appendMessagesToChat,
  createChatResponse,
  createChatResponseCommitter,
...
function shouldUseSseOverlay(policy) {
  const result = Boolean(
    policy.overlay && isProgressAllowedForCurrentPlatform()
  );
...
function resolveSubmitAssistantId() {
  return (
    chatStore.activeSession?.assistantId || chatStore.selectedAssistantId || ""
  );
}
...
function appendNormalStreamText(raw, accumulated) {
  ...
  if (normalized === DONE_STREAM_MESSAGE)
    return createDoneStreamState(accumulated);
...
async function generation(
  ...
  if (!shouldUseServerApi())
    return generationMock(url, payload, handlers, options);
    ...
    await new Promise((resolve, reject) => {
      ...
      const sourceEvent = {
        ...
        onAbort() {
          if (controller?.signal?.aborted) {
            finishReject(
              controller.signal.reason || createAbortError("Aborted")
            );
            return;
          }
          if (!completed) {
      ...
      source.addEventListener(
        "readystatechange",
        sourceEvent._onReadyStateChange
      );
...
async function createRemoteConversation({text, assistantId, modelId} = {}) {
  ...
  if (!history?.chatId) {
    logWarn(
      "[ChatHistory] new.do 응답에 chatId가 없어 요청 chatId로 대체합니다.",
      rawHistory
    );
...
async function createConversationForSubmit(normalized) {
  ...
  try {
    ...
    logWarn(
      "[ChatHistory] new.do 호출 실패, local conversation으로 대체:",
      error
    );
...
function shouldCreateConversation(targetHistoryId) {
  ...
  return (
    route.name === ROUTE_NAMES.MAIN || route.name === ROUTE_NAMES.CHAT_ENTRY
  );
}
...
async function submit(payload) {
  ...
  try {
    ...
    const {messages: nextMessages, assistantMessage} =
      appendUserAndChatResponses(targetHistoryId, normalized);
...
async function regenerate(message = {}) {
  ...
  const userMessage = findChatUserForRegenerate(
    currentMessages,
    assistantIndex
  );
...
function abortGenerationIfSwitchingConversation() {
  ...
  const currentChatId = normalizeChatId(
    resolveActiveChatId() || activeHistoryId.value
  );
