<template>
  <div class="message-actions" :class="`message-actions--${role}`">
    <button v-if="role === 'assistant'" type="button" :class="{active: feedback === 'like'}" aria-label="좋아요" @click="setFeedback('like')">좋아요</button>
    <button v-if="role === 'assistant'" type="button" :class="{active: feedback === 'dislike'}" aria-label="싫어요" @click="setFeedback('dislike')">싫어요</button>
