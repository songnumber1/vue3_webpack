  ...
  <aside class="desktop-sidebar" :class="{'desktop-sidebar--collapsed': sidebarCollapsed}">
    <div v-if="!sidebarCollapsed" class="sidebar-content sidebar-content--assistant">
      ...
      <div class="sidebar-history sidebar-history--main">
        <button v-for="item in histories" :key="item.id" class="sidebar-history-item" type="button" @click="handleSelectHistory(item)">
          {{ item.title }}
        </button>
      </div>
    ...
    <div v-else class="collapsed-sidebar" aria-label="접힌 사이드바">
      ...
      <transition name="collapsed-popover-fade">
        <section v-if="collapsedRecentOpen" class="collapsed-recent-popover" aria-label="최근 채팅 목록">
          <h2>최근 채팅</h2>
          <button v-for="item in histories" :key="item.id" class="collapsed-recent-item" type="button" @click="handleSelectHistoryCollapsed(item)">{{ item.title }}</button>
        </section>
      </transition>
  ...
  <transition name="drawer-slide">
    <aside v-if="drawerOpen" class="mobile-drawer">
      <div class="sidebar-content sidebar-content--mobile sidebar-content--assistant">
        ...
        <div class="section-label">대화</div>
        <div class="sidebar-history sidebar-history--main">
          <button v-for="item in histories" :key="item.id" class="sidebar-history-item" type="button" @click="handleSelectHistory(item)">{{ item.title }}</button>
        </div>
...
defineProps({
  ...
  activeProjectId: {type: Number, default: 0},
...
const emit = defineEmits(["update:sidebarCollapsed", "update:drawerOpen", "update:collapsedRecentOpen", "new-chat", "select-history", "select-history-collapsed"]);
...
const Icon = defineComponent({
  ...
  setup(iconProps) {
    return () => {
      const svg = h("svg", {
        class: "nav-icon", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", innerHTML: ICONS[iconProps.name] || ICONS.chat, "aria-hidden": "true",
      });
