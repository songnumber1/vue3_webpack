...
<template>
  <div class="collapsed-sidebar" aria-label="접힌 사이드바">
    <div class="collapsed-sidebar-actions">
      <button class="collapsed-icon-button" type="button" :title="t('chat.openSidebar')" :aria-label="t('chat.openSidebar')" @click="$emit('expand')">
        <Icon name="panel" bare />
      </button>
      <button class="collapsed-icon-button" type="button" :title="t('chat.newChat')" :aria-label="t('chat.newChat')" @click="$emit('new-chat')">
        <Icon name="pencil" bare />
      </button>
      <button class="collapsed-icon-button" type="button" :title="t('chat.chatSearch')" :aria-label="t('chat.chatSearch')" @click="$emit('set-recent-open', false)">
        <Icon name="search" bare />
      </button>
      <button class="collapsed-icon-button collapsed-icon-button--active" type="button" :title="t('chat.recentChats')" :aria-label="t('chat.recentChats')" @click="$emit('set-recent-open', !open)">
        <Icon name="chat" bare />
      </button>
    </div>

    <transition name="collapsed-popover-fade">
      <section v-if="open" class="collapsed-recent-popover" :aria-label="t('chat.recentChats')">
