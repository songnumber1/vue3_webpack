...
const models = [
  { id: 'assistant', label: 'Assistant', description: '일반 질의응답과 개발 작업을 처리하는 기본 Assistant' },
...
const histories = ref([
  { id: 1, title: 'Vue 슬롯 vs 컴포넌트 구조 비교 및 실제 프로젝트에서의 활용 방법과 유지보수 관점에서의 차이점 분석', preview: '슬롯과 컴포넌트 분리 기준을 실제 프로젝트 유지보수 관점에서 비교합니다.' },
  { id: 2, title: 'Compose vs Fragment 비교 및 상태 관리 흐름과 UI 재구성 시 장단점에 대한 상세 분석', preview: 'Compose와 Fragment의 상태 흐름, UI 재구성, 성능 차이를 비교합니다.' },
  { id: 3, title: 'Option API에서 Composition API로 마이그레이션 시 고려사항 및 코드 구조 개선 전략', preview: 'Composition API 전환 시 구조, 사이드 이펙트, 유지보수 기준을 정리합니다.' },
  { id: 4, title: 'Android vs React Native MVVM 아키텍처 비교 및 실제 프로젝트 적용 시 차이점', preview: 'Native와 Hybrid 구조에서 MVVM 적용 방식과 성능 차이를 비교합니다.' },
  { id: 5, title: 'npm 캐시 동기화 문제 원인 분석 및 개발 환경별 차이 해결 방법', preview: '동일 코드인데 PC별 결과가 달라지는 원인과 캐시 정리 기준을 정리합니다.' },
  { id: 6, title: '웹앱 인터뷰 질문 구성 및 실제 서비스 환경에서의 코드 설계 방식 검증', preview: '웹앱 인터뷰에서 확인할 설계, 세션, 공통 컴포넌트 구성 기준입니다.' },
  { id: 7, title: 'Spring Boot 응답 처리 구조 개선 및 공통 Response Wrapper 설계 전략', preview: 'Spring Boot 공통 응답 구조와 예외 처리 설계 방식을 정리합니다.' },
  { id: 8, title: 'Vue reactivity 내부 동작 원리 및 Proxy 기반 반응성 시스템 상세 분석', preview: 'Vue 3 reactive/ref와 Proxy 기반 반응성 동작 원리를 설명합니다.' },
  { id: 50, type: 'markdown-showcase', title: 'Markdown 통합 렌더링 50가지 샘플: KaTeX, LaTeX, Code Block, Table, Mermaid, 외부 링크', preview: '수식, 코드블록, 표 래퍼, Mermaid, 외부 링크 조합 예제가 포함됩니다.' },
])
...
async function buildMessagesFromHistory(history) {
  ...
  if (history.type === 'markdown-showcase') {
    ...
    return [
      { id: createId('message'), role: 'user', content: `${history.title} 채팅방을 열어줘` },
  ...
  return [
    ...
    { id: createId('message'), role: 'assistant', content: `${history.preview}\n\n이 화면은 저장된 대화를 선택했을 때의 샘플입니다. 실제 API나 저장소 없이 UI/UX 흐름만 재현합니다.` },
  ]
}
