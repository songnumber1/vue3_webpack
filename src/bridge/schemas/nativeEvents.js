...
export const NativeEventAckData = z.object({
  ...
  eventName: z.string().openapi({ description: "수신 이벤트명", example: "ON_APP_RESUME" })
});
...
export const OnAppResumeRequest = BaseRequest.extend({
  source: z.string().optional().openapi({ description: "이벤트 발생 주체", example: "android" })
});
export const OnAppPauseRequest = BaseRequest.extend({
  reason: z.string().optional().openapi({ description: "백그라운드 진입 사유", example: "home" })
});
...
export const NativeSelectedFile = z.object({
  ...
  uri: z.string().openapi({ description: "네이티브 파일 URI", example: "content://media/1" }),
  type: z.string().optional().openapi({ description: "MIME", example: "image/jpeg" }),
  size: z.number().optional().openapi({ description: "파일 크기", example: 12345 })
});
...
export const OnPushClickRequest = BaseRequest.extend({
  notificationId: z.string().openapi({ description: "푸시 알림 ID", example: "notice-1000" }),
  route: z.string().optional().openapi({ description: "이동 경로", example: "/notice/1000" }),
  payload: z
    .record(z.any())
    .optional()
    .openapi({
...
export const OnSessionExpiredRequest = BaseRequest.extend({
  reason: z.string().optional().openapi({ description: "만료 사유", example: "token_expired" })
});
export const OnWebViewCloseRequest = BaseRequest.extend({
  reason: z.string().optional().openapi({ description: "종료 사유", example: "app_close" })
});
...
export const OnNativeErrorRequest = BaseRequest.extend({
  error: z
    .object({
      code: z.string().openapi({ description: "오류 코드", example: "FILE_PICKER_ERROR" }),
      message: z.string().openapi({ description: "오류 메시지", example: "파일 선택 실패" }),
