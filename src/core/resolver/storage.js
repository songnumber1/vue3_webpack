...
export function resolveStorage(appInfo, bridge) {
  ...
  if (isNativeApp(appInfo)) {
    return {
      ...
      set(key, value) {
        const response = callDirectStorage(bridge, 'setStorage', {key, value: String(value)})
        if (!response?.isSuccess) console.warn('[storage] native setStorage fallback used', response?.message)
        localStorageAdapter.set(key, value)
      },
      remove(key) {
        const response = callDirectStorage(bridge, 'removeStorage', {key})
        if (!response?.isSuccess && response) console.warn('[storage] native removeStorage fallback used', response?.message)
