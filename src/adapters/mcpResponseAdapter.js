...
export function adaptMcpItem(item = {}, index = 0, options = {}) {
  ...
  return {
    ...
    categoryCode: readString(
      item,
      [M.MCP_CATEGORY_CODE],
      options.defaultCategoryCode || "COMM"
    ),
    category: readString(
      item,
      [M.MCP_CATEGORY_NAME_KO, M.MCP_CATEGORY_NAME_EN, M.MCP_CATEGORY_CODE],
    ...
    model: readString(
      item,
      [M.CONNECTOR_TYPE, M.MCP_TYPE],
      options.defaultConnector || "Connector"
    ),
    description: readString(
      item,
      [M.MCP_DESC, M.CONNECTOR_DESC],
    ...
    knowledge: readRaw(
      item,
      [M.MCP_CAPABILITY, M.MCP_AUTH_ARRAY],
