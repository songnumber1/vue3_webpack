import {RUN_ENV, PLATFORM, hasAndroidBridge, hasIosBridge, hasExtensionRuntime} from '@/core/config'

function getNavigator() { return typeof window === 'undefined' ? {} : window.navigator || {} }
function getScreen() { return typeof window === 'undefined' ? {} : window.screen || {} }
function parseVersion(ua, pattern) { const match = ua.match(pattern); return match?.[1] || '' }
...
export function resolveDetailedPlatform(baseAppInfo = {}) {
  ...
  const runtime = hasAndroidBridge() || hasIosBridge() ? RUN_ENV.NATIVE : hasExtensionRuntime() ? RUN_ENV.EXTENSION : RUN_ENV.BROWSER
  ...
  return {
    ...
    screen: { width: screen.width || 0, height: screen.height || 0, pixelRatio: typeof window === 'undefined' ? 1 : window.devicePixelRatio || 1 },
