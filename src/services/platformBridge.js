...
export async function openNativeFilePicker(options = {}) {
  ...
  return webSuccess({opened: false, reason: 'browser-file-input-required'}, '브라우저에서는 input[type=file]을 사용해야 합니다.')
}

export async function getPushToken() {
  if (!isAndroidApp()) return webSuccess({token: ''}, '브라우저에서는 FCM 토큰을 Native Bridge에서 조회하지 않습니다.')
...
export async function cancelNativeRequest(id) {
  return isAndroidApp() ? callNative('CANCEL_REQUEST', {id}) : webSuccess({id, cancelled: true})
}

export async function setBackHandler(enable) {
  return isAndroidApp() ? callNative('SET_BACK_HANDLER', {enable}) : webSuccess({enabled: false})
}
